<template>
  ...
  <div>
    ...
    <div v-if="selectedBank.length">
      ...
      <div v-else>해당 은행의 상품이 없습니다.</div>
    </div>
    ...
    <div v-else>
      ...
      <div class="sortProducts" v-if="isBestSort">
        <div
          v-for="item in store.freeSavingList"
          :key="item.fin_prdt_cd"
          class="card m-3 custom-card"
          @click="goDetailPage(item.fin_prdt_cd)"
        >
      ...
      <div class="sortProducts" v-if="!isBestSort">
        <div
          v-for="item in store.freeSavingNormalList"
          :key="item.fin_prdt_cd"
          class="card m-3 custom-card"
          @click="goDetailPage(item.fin_prdt_cd)"
        >
...
const fetchProducts = function () {
  if (selectedBank.value) {
    axios({
      method: "get",
      url: `${store.API_URL}/savings/free-saving-bank/${selectedBank.value}`,
    })
      .then((response) => {
