<template>
  ...
  <div v-if="store.freeSavingDetail.length === 0">loading</div>
  <div v-else class="container">
    <div class="row">
      <div class="first-detail col-10">
        <h4>{{ store.freeSavingDetail.kor_co_nm }}</h4>
        <h4 class="mb-5">{{ store.freeSavingDetail.fin_prdt_nm }}</h4>
      </div>
    ...
    <div class="row">
      <div class="col-2">
        ...
        <p>{{ store.freeSavingDetail["max_after"] }}%</p>
      </div>
      <div class="col-2">
        <p class="mb-1">기본 금리</p>
        <p>{{ store.freeSavingDetail["max_before"] }}%</p>
      </div>
    </div>
    <hr />
    ...
    <div class="month-calcul">
      ...
      <div class="mt-3" v-if="selectedMonth">
        ...
        <p>최고 우대 금리 {{ store.freeSavingDetail[selectedMonth] }}%</p>
        <div v-if="!isToggle">
    ...
    <div class="detail">
      ...
      <p>가입 방법 : {{ store.freeSavingDetail["join_way"] }}</p>
      <p>가입 대상 : {{ store.freeSavingDetail["join_member"] }}</p>
    </div>
  </div>
</template>
...
const submitStandard = function () {
  ...
  store.money =
    ...
    store.freeSavingDetail[selectedMonth.value] *
    0.01 *
    0.846 *
    (nowMonth.value / 12);
}
...
const formattedInterestRate = computed(() => {
  const interestRate = store.freeSavingDetail["mtrt_int"];
  if (interestRate) {
...
const filteredButtons = computed(() => {
  return Object.keys(buttons).filter((key) => store.freeSavingDetail[key] !== 0.0);
});
